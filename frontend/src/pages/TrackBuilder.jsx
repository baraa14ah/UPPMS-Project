...
export default function TrackBuilder() {
  ...
  const openOverride = async () => {
    ...
    try {
      ...
      for (const track of list) {
        const detail = trackDetails[track.id] || (await fetchTrackDetail(track.id));
        if (!detail) continue;
        const phases = detail.phases?.length
          ? detail.phases
  ...
  const handleOverride = async () => {
    if (!overrideStudentId || !overrideStageId || overrideReason.trim().length < 10) {
      toast.error(t("tracks.overrideReasonMin"));
      return;
    }
    setSaving(true);
    try {
    ...
    <Box sx={pageContainerSx}>
      ...
      <Dialog open={overrideOpen} onClose={() => setOverrideOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            <FormControl fullWidth>
              <InputLabel>{t("tracks.selectStudent")}</InputLabel>
              <Select
            ...
            <FormControl fullWidth>
              <InputLabel>{t("tracks.targetStep")}</InputLabel>
              <Select
                label={t("tracks.targetStep")}
                value={overrideStageId}
                onChange={(e) => setOverrideStageId(e.target.value)}
                disabled={overrideLoading}
              >
                {overrideStages.map((stage) => (
                  <MenuItem key={stage.id} value={stage.id}>
                    {stage.track_name}
                    {stage.sub_track_name ? ` · ${stage.sub_track_name}` : ""} · {stage.name}
                  </MenuItem>
                ))}
              </Select>
            </FormControl>
        ...
        <DialogActions>
          ...
          <Button variant="contained" color="warning" onClick={handleOverride} disabled={saving}>
