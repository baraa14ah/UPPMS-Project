...
export default function Projects() {
  ...
  const [createPhases, setCreatePhases] = useState([]);
  const [createPhasesLoading, setCreatePhasesLoading] = useState(false);
  ...
  useEffect(() => {
    if (!token || !canCreateProject || !createOpen) return;
    let cancelled = false;
    (async () => {
      setCreatePhasesLoading(true);
      try {
        const { res, data } = await apiFetch(`${API_BASE_URL}/tracks/stages-for-project`, {
          headers: authHeaders(),
        });
        if (!cancelled && res.ok) {
          setCreatePhases(Array.isArray(data?.data) ? data.data : []);
        }
      } catch {
        if (!cancelled) setCreatePhases([]);
      } finally {
        if (!cancelled) setCreatePhasesLoading(false);
      }
    })();
    return () => {
      cancelled = true;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [token, canCreateProject, createOpen]);

  /** Creates a new project and refreshes the list on success. */
  const handleCreateProject = async ({ title, description, track_phase_id }) => {
    try {
      setCreating(true);
      const body = { title, description };
      if (track_phase_id) {
        body.track_phase_id = track_phase_id;
      }
      ...
      if (!res.ok) {
        toast.error(data?.message || data?.errors?.track_phase_id?.[0] || t("projects.loadError"));
        return false;
      }
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <CreateProjectDialog
        ...
        submitting={creating}
        t={t}
        phaseOptions={createPhases}
        phasesLoading={createPhasesLoading}
      />
    </Box>
  );
}
