...
class TrackService
{
    ...
    public function resolveProjectTrackStageId(Project $project): ?int
    {
        ...
        $stageId = $project->track_stage_id ?: $project->proposal?->track_stage_id;
    ...
    public function describeProjectTrackStage(Project $project): ?array
    {
        ...
        $stage = $project->trackStage ?: $project->proposal?->trackStage;
        if (!$stage) {
            return null;
        }
        ...
        // Flat steps without a phase container still show the step name.
        if (!$phaseName) {
            return [
                'id' => $stage->id,
    ...
    {
        $projectStep ??= $project->trackStage ?: $project->proposal?->trackStage;
    ...
    public function assignStudentsToTrack(Track $track, array $studentIds, bool $confirmReassign = false): array
    {
        ...
        $firstStage = TrackStageHierarchy::flattenedActionableSteps($track)->first();
        if (!$firstStage) {
            throw ValidationException::withMessages([
                'track' => ['Track must have at least one actionable step before assigning students.'],
            ]);
        }
    ...
    public function getStudentProgress(User $student): array
    {
        ...
        $flatSteps = TrackStageHierarchy::flattenedActionableSteps($track)->values();
        $this->syncCatchUpPassedSteps($student, $track, $flatSteps);
        ...
        $progressRecords = StudentProgress::query()
            ...
            ->withCount('history')
            ->get()
            ->keyBy(fn (StudentProgress $p) => (int) $p->track_stage_id);
        ...
        return [
            ...
            'completion_percent' => $flatSteps->isEmpty()
                ? 0
                : (int) round(
                    (collect($timeline)->where('status', 'passed')->count() / $flatSteps->count()) * 100
                ),
            'stages_passed' => collect($timeline)->where('status', 'passed')->count(),
            'stages_total' => $flatSteps->count(),
            'timeline' => $timeline,
            'phases' => $phases,
            'history' => $history,
        ];
    ...
    /**
     * Heals progress after override:
     * - If student is in_progress on step N, ensure 0..N-1 are passed.
     * - Legacy overrides left the target as in_progress (not passed); complete that
     *   step and open the next one so "إنجاز المسار" moves.
     *
     * @param  Collection<int, TrackStage>  $flatSteps
     */
    private function syncCatchUpPassedSteps(User $student, Track $track, Collection $flatSteps): void
    {
        if ($flatSteps->isEmpty()) {
            return;
        }

        $records = StudentProgress::query()
            ->where('student_id', $student->id)
            ->where('track_id', $track->id)
            ->with('history')
            ->get()
        ...
        $inProgressIndexes = [];
        foreach ($flatSteps as $index => $step) {
            if (($records->get((int) $step->id)?->status) === 'in_progress') {
                $inProgressIndexes[] = (int) $index;
            }
        }

        if ($inProgressIndexes === []) {
            return;
        }

        $currentIndex = min($inProgressIndexes);
        /** @var TrackStage $currentStep */
        $currentStep = $flatSteps[$currentIndex];
        $currentRecord = $records->get((int) $currentStep->id);

        $legacyOverrideUnlock = $currentRecord
            && ! $currentRecord->defense_result_recorded_at
            && $currentRecord->history->contains(
                fn ($h) => $h->status === 'in_progress' && filled($h->modification_reason),
            );

        // Complete through current when it was only "unlocked" by an older override.
        $passThroughIndex = $legacyOverrideUnlock ? $currentIndex : ($currentIndex - 1);

        for ($i = 0; $i <= $passThroughIndex; $i++) {
            $prior = $flatSteps[$i] ?? null;
            if (!$prior) {
                continue;
            }
            $existing = $records->get((int) $prior->id);
        ...
        if ($legacyOverrideUnlock) {
            $next = $flatSteps->get($currentIndex + 1);
            if ($next) {
                $nextExisting = $records->get((int) $next->id);
                if (! $nextExisting || $nextExisting->status !== 'passed') {
                    StudentProgress::updateOrCreate(
                        [
                            'student_id' => $student->id,
                            'track_id' => $track->id,
                            'track_stage_id' => $next->id,
                        ],
                        [
                            'status' => 'in_progress',
                            'completed_at' => null,
    ...
    private function buildTimelineEntry(
        ...
        $record = $progressRecords->get((int) $stage->id);
        $previous = TrackStageHierarchy::previousActionableStep($stage);
        $previousPassed = !$previous
            || ($progressRecords->get((int) $previous->id)?->status === 'passed');
    ...
    /**
     * Main academic stations (phases) for admin project creation.
     * Selecting a phase places the project on that phase's first step.
     *
     * @return array<int, array{id:int,track_id:int,track_name:string,name:string,label:string,first_step_id:?int}>
     */
    public function getStagesForProjectCreate(int $universityId): array
    {
        $tracks = Track::query()
            ->where('university_id', $universityId)
            ->where('is_active', true)
            ->with(['stages.academicStage', 'stages.parent'])
            ->orderBy('name')
            ->get();
        ...
        foreach ($tracks as $track) {
            foreach (TrackStageHierarchy::groupedPhases($track) as $group) {
                ...
                $firstStep = $group['steps']->first();
                $options[] = [
                    'id' => $phase->id,
                    'track_id' => $track->id,
                    'track_name' => $track->name,
                    'name' => $phase->name,
                    'label' => trim($track->name . ' · ' . $phase->name),
    ...
    public function recordDefenseResult(
        ...
        [$session, $project, $owner, $trackStage] = $this->resolveDefenseRecordingContext($session, $recorder);
        ...
        return $this->applyDefenseProgressForProject($session, $recorder, $project, $trackStage, $result, $reason);
    }
    ...
    public function completeNonDecisiveStage(
        ...
        return $this->applyDefenseProgressForProject($session, $recorder, $project, $trackStage, 'passed', $reason);
    }
    ...
    public function autoCompleteExpiredNonDecisiveSessionIfNeeded(DefenseSession $session): bool
    {
        ...
        $this->applyDefenseProgressForProject(
            $session,
            $recorder,
            $session->project,
            $trackStage,
    ...
    /**
     * Applies defense/stage progress to the project owner and all accepted members
     * so every teammate's "track completion" advances together.
     */
    private function applyDefenseProgressForProject(
        DefenseSession $session,
        User $recorder,
        Project $project,
        ...
        $students = collect([$project->user])
            ->merge(
                $project->members->filter(
                    fn ($member) => ($member->pivot->status ?? null) === 'accepted'
                )
            )
            ->filter()
            ->unique('id')
        ...
        if ($students->isEmpty()) {
            throw ValidationException::withMessages([
                'student' => ['No students found on this project.'],
            ]);
        }

        return DB::transaction(function () use ($students, $session, $recorder, $project, $trackStage, $result, $reason) {
            $ownerResult = null;
            foreach ($students as $student) {
                $payload = $this->applyDefenseProgress(
                    $session,
                    $recorder,
                    $project,
                    $student,
                ...
                if ((int) $student->id === (int) $project->user_id || $ownerResult === null) {
                    $ownerResult = $payload;
                }
            }
    ...
    private function applyDefenseProgress(
        ...
        return DB::transaction(function () use ($progress, $recorder, $result, $reason, $trackStage, $student, $lateModification, $session, $project) {
            ...
            $this->notifications->notifyUser(
                ...
                [
                    'track_stage_id' => $trackStage->id,
                    'stage_name' => $trackStage->academicStage?->name ?? $trackStage->name,
                    'phase_name' => $trackStage->parent?->name,
                    'result' => $result,
                    'track_completed' => $trackCompleted,
                    'phase_completed' => $phaseCompleted,
                    'graduated' => $graduated,
                    'next_stage_id' => $nextStage?->id,
                    'next_stage_name' => $nextStage
                        ? ($nextStage->academicStage?->name ?? $nextStage->name)
                        : null,
    ...
    public function overridePrerequisite(User $student, int $trackStageId, string $reason, User $admin): StudentProgress
    {
        ...
        $stage = TrackStage::with(['track.stages.academicStage', 'academicStage'])->findOrFail($trackStageId);
        ...
        try {
            TrackStageHierarchy::assertActionable($stage);
        } catch (\InvalidArgumentException $e) {
            throw ValidationException::withMessages([
                'track_stage_id' => [$e->getMessage()],
            ]);
        }
        ...
        return DB::transaction(function () use ($student, $stage, $reason, $admin) {
            ...
            $flat = TrackStageHierarchy::flattenedActionableSteps($track)->values();
            $targetIndex = $flat->search(fn (TrackStage $s) => (int) $s->id === (int) $stage->id);

            if ($targetIndex === false) {
                throw ValidationException::withMessages([
                    'track_stage_id' => ['Selected step is not part of this track progression.'],
                ]);
            }

            // Complete through the selected step (inclusive) so it becomes "منجزة"
            // and the progress bar advances; then open the next step if any.
            $completedProgress = null;
            for ($i = 0; $i <= $targetIndex; $i++) {
                /** @var TrackStage $step */
                $step = $flat[$i];
                $stepProgress = StudentProgress::query()->firstOrNew([
                    'student_id' => $student->id,
                    'track_id' => $track->id,
                    'track_stage_id' => $step->id,
                ]);
                $alreadyPassed = $stepProgress->exists && $stepProgress->status === 'passed';

                $stepProgress->fill([
                    'status' => 'passed',
                    'completed_at' => $stepProgress->completed_at ?? now(),
                ]);
                $stepProgress->save();

                if (! $alreadyPassed) {
                    $this->logProgressHistory($stepProgress->fresh(), $admin, 'passed', $reason);
                }

                if ($i === $targetIndex) {
                    $completedProgress = $stepProgress->fresh(['trackStage']);
                }
            }

            $next = $flat->get($targetIndex + 1);
            if ($next) {
                $nextProgress = StudentProgress::query()->firstOrNew([
                    'student_id' => $student->id,
                    'track_id' => $track->id,
                    'track_stage_id' => $next->id,
                ]);
                if (! $nextProgress->exists || $nextProgress->status !== 'passed') {
                    $nextProgress->fill([
                        'status' => 'in_progress',
                        'completed_at' => null,
                    ]);
                    $nextProgress->save();
                    $this->logProgressHistory($nextProgress->fresh(), $admin, 'in_progress', $reason);
                }
            ...
            $this->notifications->notifyUser(
                ...
                sprintf(
                    'An administrator completed «%s» for you via prerequisite override: %s',
                    $stage->academicStage?->name ?? $stage->name,
                    $reason,
                ),
                [
                    'track_stage_id' => $stage->id,
            ...
            return $completedProgress ?? StudentProgress::query()
                ->where('student_id', $student->id)
                ->where('track_stage_id', $stage->id)
                ->firstOrFail();
        });
    ...
    private function resolveTrackStageForDefense(DefenseSession $session, User $student): ?TrackStage
    {
        ...
        if ($session->project?->track_stage_id) {
            return TrackStage::find($session->project->track_stage_id);
        }
