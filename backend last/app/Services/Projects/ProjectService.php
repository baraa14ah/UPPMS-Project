...
class ProjectService
{
    ...
    public function create(Request $request): Project
    {
        ...
        $data = $request->validate([
            ...
            'track_phase_id'   => 'nullable|integer|exists:track_stages,id',
        ]);

        $universityId = (int) $user->university_id;

        $phaseId = isset($data['track_phase_id']) ? (int) $data['track_phase_id'] : null;
        $stageId = $phaseId
            ? $this->resolveFirstStepIdForPhase($phaseId, $universityId)
            : $this->resolveFirstAcademicStageId($universityId);
        ...
        $payload = [
            ...
            'status' => 'pending',
            'track_stage_id' => $stageId,
        ];

        if (!empty($data['supervisor_id'])) {
            $payload['supervisor_id'] = (int) $data['supervisor_id'];
        }
    ...
    /**
     * Place the project on the first actionable step inside the chosen academic phase.
     */
    private function resolveFirstStepIdForPhase(int $phaseId, int $universityId): int
    {
        $phase = TrackStage::query()
            ->with(['track', 'children' => fn ($q) => $q->orderBy('sequence_order')])
            ->whereKey($phaseId)
            ->whereHas('track', fn ($q) => $q->where('university_id', $universityId))
            ->first();

        if (!$phase || $phase->stage_kind !== TrackStageHierarchy::KIND_PHASE) {
            throw ValidationException::withMessages([
                'track_phase_id' => ['Select a main academic station (phase), not a step.'],
            ]);
        }

        $firstStep = $phase->children
            ->first(fn (TrackStage $child) => $child->stage_kind === TrackStageHierarchy::KIND_STEP);

        if (!$firstStep) {
            // Fallback: load via hierarchy if children relation empty
            $track = Track::query()
                ->with(['stages.academicStage', 'stages.parent'])
                ->find($phase->track_id);
            foreach (TrackStageHierarchy::groupedPhases($track) as $group) {
                if ((int) ($group['phase']?->id) === (int) $phase->id) {
        ...
        if (!$firstStep) {
            throw ValidationException::withMessages([
                'track_phase_id' => ['This academic station has no steps yet.'],
            ]);
        }

        return (int) $firstStep->id;
    }

    /** First actionable step of the first active university track, if any. */
    private function resolveFirstAcademicStageId(int $universityId): ?int
    {
        $track = Track::query()
        ...
        return TrackStageHierarchy::flattenedActionableSteps($track)->first()?->id;
    }
    ...
    public function listForIndex(User $user): Collection
    {
        ...
        $query = Project::query()->with([
            ...
            'trackStage:id,name,parent_id,academic_stage_id,track_id',
    ...
    public function attachTrackStageMetadata(Collection $projects): Collection
    {
        ...
        return $projects->map(function (Project $project) use ($trackService) {
            return $this->applyTrackStageAttribute(
                $project,
    ...
    /**
     * Store computed track_stage payload and hide the Eloquent relation so JSON
     * does not overwrite phase_name / display_label with the raw TrackStage model.
     */
    private function applyTrackStageAttribute(Project $project, ?array $description): Project
    {
        $project->unsetRelation('trackStage');
        if ($project->relationLoaded('proposal') && $project->proposal) {
            $project->proposal->unsetRelation('trackStage');
        }

        $project->setAttribute('track_stage', $description);
